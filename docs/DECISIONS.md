...
---
## ADR-032 — Gates are evaluated in order, but the outcome is resolved by severity
**Context.** docs/POLICY_SPEC.md §5 states two things that pull in different directions. "Ordering is load-bearing. Gate 5 runs before gates that could produce contact, so a fraud-flagged case can never be messaged regardless of what came earlier." And TASK 4: "ALL results persist to `plans.guardrail_results` — passes included, not just failures." A chain that short-circuits on the first failure satisfies the first and breaks the second: `guardrail_results` becomes a partial record, and "we stopped looking" is weaker evidence than "we checked all eight". But a chain that resolves the outcome positionally would let `contact_cap` (gate 3) decide a *downgrade* on a case that `terminal_check` (gate 5) would have *stopped*, leaving a terminal case alive.
**Options.** (a) Short-circuit on the first failure. (b) Evaluate all eight and take the first failure positionally. (c) Evaluate all eight, and resolve the disposition by severity.
**Chosen.** (c). Gates run in the policy.yaml order 0→7 so the persisted record reads in order, and the disposition is then resolved most-severe-first: `kill_switch` → `stop_case` → `drop_and_log` → `escalate_human` → `downgrade_to_non_contact` → reschedule → allow.
**Reason.** This makes "a terminal case is never contacted" a GUARANTEE rather than a property of gate numbering. It also produces a strictly better audit record: an operator can see that a case was stopped for fraud *and* that it would additionally have breached the contact cap. Verified on real data: `terminal_check` failed 57 times in the 500-case batch and decided all 57 outcomes, out-ranking 36 quiet-hours failures and 5 amount-ceiling failures on the same cases.
**Trade-offs.** Slightly more work per plan — eight gate evaluations even when the first one stops the case. At 500 cases the cost is unmeasurable, and the gates are pure functions over already-loaded state.
**Consequences.** Reschedules are applied last and cumulatively, because a plan can be both inside the cooling window and inside quiet hours; the later boundary wins. An unrecognised `on_fail` string falls through to `dropped` with `anyFailedClosed: true` — a future policy.yaml could name a disposition this code does not know, and "unknown instruction" must mean "do nothing".

---
## ADR-033 — Every gate fails closed, and a fail-closed block is distinguishable
**Context.** Run 3 shipped a gate that failed OPEN. `scoreFromGuardOutput` parsed an empty Prompt Guard response with `Number('')`, which is `0`, so "no data" read as "definitely benign" and the case was waved through. The bug survived because a working heuristic fallback hid it.
**Options.** (a) Validate inputs at the chain boundary once. (b) Make every gate independently defensive.
**Chosen.** (b), plus a distinct `failedClosed: true` marker on the verdict.
**Reason.** (a) puts the whole chain's safety in one place that a future caller can bypass. Per-gate defensiveness means a gate is safe no matter how it is invoked, which matters because Run 5 will call this chain a second time at execution with freshly loaded state. The marker exists because an operator needs to tell two situations apart: "policy says no" is a decision, and "we could not tell, so we refused" is a data problem that needs fixing. Both block the action; only one needs a human.
**Trade-offs.** Repetitive validation, and `as never` casts in the tests to feed values the type system forbids but a database, a JSON column, or a hand-edited policy.yaml can still deliver.
**Consequences.** 44 dedicated hostile-input assertions: every gate fed empty string, `null`, `undefined`, and `NaN` in the field it depends on, plus malformed policy thresholds. Notable specifics — an unparseable `quiet_hours.start` refuses outreach rather than assuming none is restricted; a missing `root_cause` cannot be *proven* non-terminal so it stops; a non-integer amount is rejected because money is never a float; and a malformed cost falls back to `human_escalation_paise` rather than 0, because failing closed on money means assuming an action is EXPENSIVE.

---
## ADR-034 — All scheduling delays pass through one helper
**Context.** `DEMO_TIME_SCALE` compresses every delay for the demo video. `docs/CLAUDE_CONTEXT.md` has carried the same warning since Run 1: "One central helper; no call site computes a delay independently. Retrofitting means touching every call site."
**Options.** (a) Apply the scale at each call site. (b) Apply it once, in a single helper every path must use.
**Chosen.** (b): `scaleDelayMs(delayMs, scale)` and `scheduleAfter(now, delayMs, scale)` in `timing/clock.ts`. Nothing else divides by the scale, and `scheduleAfter` is the only way a future instant is produced.
**Reason.** A scale honoured by three of four code paths is worse than one honoured by none: the demo would show some delays compressed and others not, and the discrepancy would look like a bug in the agent rather than in the plumbing.
**Trade-offs.** Every strategy must thread `demoTimeScale` through its context object.
**Consequences.** `scaleDelayMs` also fails closed: a zero, negative, fractional, `NaN`, or infinite scale falls back to 1 (real timing), because a scale that cannot be trusted must slow the system down, never speed it up. A test asserts that the static table, the salary window, and the downtime wait are all compressed at 360×, which is what makes the guarantee checkable rather than aspirational.

---
## ADR-035 — An open outage produces a re-check, not a retry
**Context.** Run 3 added `downtime_windows` and the `downtime_signal` diagnosis path. TASK 3 asked for the payoff: schedule a retry relative to `resolved_at` rather than a guessed +2h. But that leaves the case where the outage is still OPEN and there is no `resolved_at` to schedule against.
**Options.** (a) Retry at the static +2h anyway. (b) Retry immediately and let it fail. (c) Schedule a silent re-check and take no money action until the outage clears.
**Chosen.** (c). `TimingDecision.recheckOnly` marks it, and the policy engine downgrades the action to a silent `delayed_retry` at zero cost, zero expected recovery, and no contact.
**Reason.** An attempt against a bank Razorpay has *told us* is down is guaranteed to fail, and it would consume one of the three attempts gate 1 allows — spending a finite budget on a certain failure. This is the concrete value of the downtime signal over inference: not just better timing, but knowing when *not* to act.
**Trade-offs.** An outage whose `.resolved` webhook never arrives leaves the case re-checking indefinitely. Bounded in practice by gate 1, and `findStaleOpenWindows` (Run 3) already surfaces any window open beyond 24h.
**Consequences.** A resolved window schedules at `resolved_at + 0.5h`, or immediately if that instant has already passed — waiting a nominal 2h after an outage has demonstrably ended would delay a recovery for no reason. The bandit deliberately does NOT override either downtime path: exploration is for tuning an ordinary delay, not for ignoring a known outage. Verified against a real `downtime_windows` row in `pnpm plan:smoke`, both open and resolved.

---
## ADR-036 — Terminal causes produce a `stop` plan, not the absence of a plan
**Context.** The first implementation gave terminal causes `maxAttempts: 0`, so `interventionForAttempt` returned null and no plan row was written at all. Three tests failed and exposed the flaw: TASK 1 requires every one of the 21 causes to map to something, "including terminal causes, which map to stop".
**Options.** (a) Write no plan for a terminal case. (b) Write a `stop` plan recording the decision.
**Chosen.** (b), `maxAttempts: 1`.
**Reason.** "We deliberately did nothing, and here is why" is a decision that belongs in the audit trail. The absence of a row is indistinguishable from a case the planner never reached, which is exactly the ambiguity `plans` exists to remove — and it would leave the dashboard unable to show that a fraud-flagged case was handled correctly rather than dropped on the floor. A second call still returns null, because one stop is enough.
**Consequences.** All 57 terminal cases in the 500-case batch carry a persisted plan with `action_type = 'stop'`, `status = 'dropped'`, all eight gate verdicts, and an `exceptions` row naming `terminal_check`. Confirmed by row-level query rather than by counter.
